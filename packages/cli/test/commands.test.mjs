...
describe('forge generate — runtime hooks (issue #32)', () => {
  ...
  test('Kiro: generated files carry the forge marker; manual files survive --force', (t) => {
    const dir = makeTmpDir(t);
    writeProjectYaml(dir, ALL_RUNTIMES_YAML);
    ...
    for (const f of ['product.md', 'structure.md', 'agents.md', 'commands.md']) {
      assert.ok(
        readFileSync(join(steering, f), 'utf-8').includes('Generado por forge'),
        `.kiro/steering/${f} must carry the forge marker`,
      );
    }
    const kiroHooks = join(dir, '.kiro', 'hooks');
    for (const f of ['pre-edit-branch-guard.json', 'pre-bash-check.json', 'post-turn-check.json']) {
      assert.ok(
        readFileSync(join(kiroHooks, f), 'utf-8').includes('Generated by forge'),
        `.kiro/hooks/${f} must carry the forge marker`,
      );
    }

    // A hand-written steering file (no marker) must never be overwritten, even with --force.
    const manual = '# Mi contexto de producto escrito a mano\n';
    writeFileSync(join(steering, 'product.md'), manual);
    const { status, stdout } = runForge(['generate', '--runtime', 'kiro', '--force'], { cwd: dir });
    assert.equal(status, 0, stdout);
    assert.equal(
      readFileSync(join(steering, 'product.md'), 'utf-8'), manual,
      'manual .kiro/steering/product.md was overwritten by generate --force',
    );
  });
