...
Options:
  ...
  --force            Overwrite files generated by forge (manual files without
                     the forge marker are never overwritten)
...
/** Marcadores que identifican los archivos que forge generó y puede regenerar. */
const FORGE_MARKERS = ['Generado por forge', 'Generated by forge'];

function hasForgeMarker(text: string): boolean {
  return FORGE_MARKERS.some(m => text.includes(m));
}

function writeFile(path: string, content: string, dryRun: boolean, force: boolean): string {
  if (dryRun) return 'DRY-RUN';
  if (existsSync(path)) {
    if (!force) return 'SKIP (usa --force para sobreescribir)';
    // Guard de archivos manuales: si esta surface lleva marcador de forge pero
    // el archivo existente no lo tiene, fue escrito a mano — NUNCA se
    // sobrescribe, ni siquiera con --force.
    if (hasForgeMarker(content) && !hasForgeMarker(readFileSync(path, 'utf-8'))) {
 ...
 * sobrescribe, ni siquiera con --force (además avisa por consola).
 */
function writeNestedAgentsMd(path: string, content: string, dryRun: boolean, force: boolean): string {
  if (existsSync(path) && !hasForgeMarker(readFileSync(path, 'utf-8'))) {
    console.warn(yellow(
