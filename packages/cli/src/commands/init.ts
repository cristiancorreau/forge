...
export function installHooks(forgeRoot: string, destDir: string, mode: string, force: boolean, approvalsEnabled = false): void {
  mkdirSync(destDir, { recursive: true });
  ...
  // Approval gate (SPEC-083 P6): si settings.json va a registrar el hook
  // (approvals.enabled: true), el archivo TIENE que instalarse en el mismo
  // paso — registro sin instalación rompería cada tool call. Un
  // pre-approval-gate.cjs editado a mano (sin marcador forge) nunca se pisa,
  // ni con --force (misma regla que forge generate). Extensión .cjs: el hook
  // es CommonJS y debe correr también en proyectos con "type": "module".
  if (approvalsEnabled) {
    const src = join(hooksDir, 'pre-approval-gate.cjs');
    const dest = join(destDir, 'pre-approval-gate.cjs');
    if (existsSync(src)) {
      const isManual = existsSync(dest) &&
        !readFileSync(dest, 'utf-8').includes('Generated by forge');
      if (!isManual) copyFile(src, dest, force);
    }
  }
}
...
function buildSettings(language: string, mode: string, approvalsEnabled = false): Record<string, unknown> {
  ...
  if (approvalsEnabled) {
    (hooks.PreToolUse as Array<Record<string, unknown>>).push({
      ...
      hooks: [{ type: 'command', command: 'node .claude/hooks/pre-approval-gate.cjs' }],
    });
  }
  ...
  if (runtime === 'claude-code') {
    ...
    await runTasks('Installing...', [
      ...
      },
      {
        ...
        tech: 'guardrail',
        task: () => installHooks(forgeRoot, join(claudeDir, 'hooks'), mode, force, config.approvals?.enabled === true),
      },
      {
      ...
      },
      {
        ...
        // approvals.enabled viaja también acá: sin él, un init --force sobre un
        // proyecto con approvals borraría el registro del gate (mergeSettings
        // reemplaza `hooks` por completo) hasta el próximo forge generate.
        task: () => writeSettingsJson(join(claudeDir, 'settings.json'), language, mode, force, config.approvals?.enabled === true),
      },
      {
