...
export function installHooks(forgeRoot: string, destDir: string, mode: string, force: boolean, approvalsEnabled = false): void {
  ...
  // pre-approval-gate.js editado a mano (sin marcador forge) nunca se pisa,
  // ni con --force (misma regla que forge generate).
  if (approvalsEnabled) {
    const src = join(hooksDir, 'pre-approval-gate.js');
    const dest = join(destDir, 'pre-approval-gate.js');
    if (existsSync(src)) {
