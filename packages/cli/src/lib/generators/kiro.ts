...
/**
 * Marcador de archivo generado (mismo texto que reconoce el guard de
 * `forge generate`): sin él, un archivo escrito a mano en .kiro/ sería
 * sobrescrito por `generate --force`.
 */
const KIRO_MD_MARKER = '> Generado por forge. Actualizar project.yaml para cambiar la configuración.';
...
export function generateKiroBranchGuardHook(): string {
  return JSON.stringify({
    ...
    description: 'Warn before editing files on main or master branch (Generated by forge)',
    event: 'PreEdit',
    condition: {
      type: 'script',
...
export function generateKiroBashCheckHook(): string {
  return JSON.stringify({
    ...
    description: 'Block destructive commands (DROP TABLE, --force-reset, rm -rf /, git push --force) (Generated by forge)',
    event: 'PreBash',
    condition: {
      type: 'script',
...
export function generateKiroPostTurnHook(): string {
  return JSON.stringify({
    ...
    description: 'After each turn, warn about debug statements left in changed files (Generated by forge)',
    event: 'PostTurn',
    condition: {
      type: 'script',
